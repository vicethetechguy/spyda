...
export default async function handler(req: any, res: any) {
  ...
  return res.status(200).json({
    ...
    visionAdapter: process.env.SPYDA_VISION_SERVICE_URL ? 'roboflow-supervision' : 'document-ocr',
    hasVisionServiceToken: Boolean(process.env.SPYDA_VISION_SERVICE_TOKEN),
    imageModel,
    analysisModel,
    groqAnalysisModel,
  });
