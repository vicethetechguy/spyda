      ...
      <div className="relative z-10 w-full max-w-[420px] p-8">
        <div className="text-center mb-10">
          ...
          <h1 className="font-heading text-3xl font-bold text-foreground mb-2">
            {mfaStatus === 'required' ? 'Confirm it is you' : isSignUp ? 'Create your account' : 'Welcome back'}
          </h1>
          <p className="text-sm text-muted-foreground">
            {mfaStatus === 'required' ? 'Enter the code from your authenticator app' : isSignUp ? 'Enter your details to get started with Spyda' : 'Enter your credentials to access your workspace'}
          </p>
        </div>

        <form onSubmit={mfaStatus === 'required' ? handleMfa : handleAuth} className="space-y-5">
          {error && (
          ...
          {mfaStatus === 'required' ? (
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-12 pl-10 pr-4 rounded-xl bg-white/[0.03] border border-white/[0.08] text-sm tracking-[0.2em] text-foreground focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all outline-none"
              />
            </div>
          ...
          >
            {loading || mfaStatus === 'checking' ? <Loader2 className="w-5 h-5 animate-spin" /> : mfaStatus === 'required' ? 'Verify code' : isSignUp ? 'Sign Up' : 'Sign In'}
            {!loading && mfaStatus !== 'checking' && (mfaStatus === 'required' ? <ShieldCheck className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />)}
          </button>
        </form>

        {mfaStatus !== 'required' && <div className="mt-8 text-center text-sm text-muted-foreground">
